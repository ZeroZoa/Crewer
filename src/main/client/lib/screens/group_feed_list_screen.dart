...
/// 피드 리스트 화면
class GroupFeedListScreen extends StatefulWidget {
  @override
  _GroupFeedListScreenState createState() => _GroupFeedListScreenState();
}

class _GroupFeedListScreenState extends State<GroupFeedListScreen> {
  List<dynamic> feeds = [];
  int page = 0;
  bool hasMore = true;
  bool loading = false;
  ...
  Future<void> _fetchPopularFeeds() async {
    ...
    try {
      final response = await http.get(
        Uri.parse('${ApiConfig.baseUrl}${ApiConfig.getGroupFeedListPopular()}?page=$page&size=20'),
      );
  ...
  Future<void> _fetchNewFeeds() async {
    ...
    try {
      final response = await http.get(
        Uri.parse('${ApiConfig.baseUrl}${ApiConfig.getGroupFeedListNew()}?page=$page&size=20'),
      );
                  ...
                  child: ListView.separated(
                    ...
                    itemBuilder: (ctx, idx) {
                      ...
                      return GestureDetector(
                        onTap: () {
                          final route = '/groupfeeds/${feed['id']}';
                          context.push(route);
                        },
                        child: Container(
                          decoration: BoxDecoration(
